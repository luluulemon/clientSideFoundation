...
public class GameRepository {
    ...
    public JsonArray getGame(int limit, int offset, String ng){
        Query query = new Query().limit(limit).skip(offset);
        query.fields().include("name", "gid");
        List<Document> results = mongoTemplate.find(query, Document.class, "games");
        JsonArrayBuilder gameArrBuilder = Json.createArrayBuilder();
        results.stream().forEach(doc -> gameArrBuilder.add(Game.toJSON(doc)));
        return gameArrBuilder.build();
    }
